...
class MainViewModel : ViewModel() {
    ...
    private val _plantResponse: MutableLiveData<Boolean> = MutableLiveData()
    val plantResponse: LiveData<Boolean>
        get() = _plantResponse

    fun m() = viewModelScope.launch(Dispatchers.IO) {
        _plantResponse.value = true
        _plantResponse.postValue(true)
        val oA = async { a() }
        val oB = async { b() }
        val oC = async { c() }
